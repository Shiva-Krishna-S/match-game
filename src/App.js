...
class App extends Component {
  ...
  tick = () => {
    ...
    if (secondsElapsed === 1) {
      this.setState({secondsElapsed: 0, isGameOver: true})
      clearInterval(this.timerId)
    } else {
  ...
  onImageClick = id => {
    const {toMatchImgUrlId} = this.state
    const randomId = Math.ceil(Math.random() * (imagesList.length - 1))
    if (toMatchImgUrlId === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        toMatchImgUrlId: imagesList[randomId].id,
      }))
    } else {
  ...
  render() {
    ...
    const filteredImages = this.getActiveTabImages()

    const toMatchImgObj = imagesList.find(
      eachImage => eachImage.id === toMatchImgUrlId,
    )
    const toMatchImgUrl = toMatchImgObj.imageUrl
        ...
        {!isGameOver && (
          <div className="app-content-container">
            <div className="match-image-container">
              <img className="match-image" src={toMatchImgUrl} alt="match" />
            </div>
